...
import { controllerMatrix, buttonState } from "../render/core/controllerInput.js";
   ...
   export const init = async model => {
      ...
      beamL.add('tubeX').color(1,0,0).move(0,0,-10).scale(.001,.001,10);

      let beamR = model.add();
      beamR.add('cube').color(1,0,0).move(.02,0,0).scale(.02,.005,.005);
      beamR.add('cube').color(0,0,1).move(0,.02,0).scale(.005,.02,.005);
      beamR.add('tubeX').color(0,1,0).move(0,0,-10).scale(.001,.001,10);
   }
   ...
   export const display = model => {
	 ...
	 // CHECK TO SEE WHETHER EACH BEAM INTERSECTS WITH THE TARGET
         console.log(cg.mHitRectXZ(LM, target.getMatrix()))
         console.log(cg.mHitRectXZ(RM, target.getMatrix()))
         let hitL = cg.mHitRectXY(LM, target.getMatrix())!== null && cg.mHitRectYZ(LM, target.getMatrix())!== null && cg.mHitRectXZ(LM, target.getMatrix())!== null;
         let hitR = cg.mHitRectXY(RM, target.getMatrix())!== null && cg.mHitRectYZ(RM, target.getMatrix())!== null && cg.mHitRectXZ(RM, target.getMatrix())!== null;
